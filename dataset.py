...
def get_data_dict(feature_dir, label_dir, video_list, event_list, sample_rate=4, temporal_aug=True,
                  boundary_smooth=None):  # 비디오 별 feature 및 label,boundary 정보를 불러와 torch.tensor 형태로 저장
...
def restore_full_sequence(x, full_len, left_offset, right_offset,
                          sample_rate):  # 다운샘플링된 시퀀스를 원래 시퀀스 길이로 interpolation 하여 복원
