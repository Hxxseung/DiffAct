...
class LayerNorm1d(nn.Module):
    ...
    def forward(self, x):
        ...
        mean = x.mean(dim=1, keepdim=True)  # (batch, 1, time)
        var = x.var(dim=1, keepdim=True, unbiased=False)  # (batch, 1, time)
...
class MixedConvAttentionLayer(nn.Module):
    def __init__(self, d_model, kernel_size, dilation, dropout_rate):
        ...
        # Custom 1D LayerNorm
        self.norm_conv = LayerNorm1d(d_model)
        self.norm_att = LayerNorm1d(d_model)
        self.norm_ffn = LayerNorm1d(d_model)
    ...
    def forward(self, x):
        ...
        combined = out1 + out2 + x

        # Pre-norm for FFN
        combined_norm = self.norm_ffn(combined)
        out = self.ffn_block(combined_norm)
